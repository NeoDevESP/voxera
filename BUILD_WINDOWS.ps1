...
$projectConfig = Get-Content (Join-Path $PSScriptRoot 'CMakeLists.txt') -Raw
if ($projectConfig -notmatch 'project\(VOXERA VERSION ([0-9.]+)') { throw 'Project version missing' }
$appVersion = $Matches[1]
$installerName = "VOXERA-1.5-$appVersion-Windows-x64-Setup.exe"
...
if ($Package) {
    ...
    if (Test-Path "dist/$installerName") {
        Remove-Item "dist/$installerName"
    }
}
Write-Host "VOXERA v$appVersion - Windows x64: build, tests, plugin validation"
...
Invoke-Checked $validator.FullName @("--strictness-level", "5", "--skip-gui-tests", "--random-seed", "12345", "--validate", "$PSScriptRoot/build/VOXERA_artefacts/Release/VST3/VOXERA 1.5.vst3")
if ($Package) {
    Invoke-Checked $iscc @("/DAppVersion=$appVersion", "Installer/VOXERA.iss")
...
} else { Write-Host "Validated VST3: build/VOXERA_artefacts/Release/VST3/VOXERA 1.5.vst3" }
